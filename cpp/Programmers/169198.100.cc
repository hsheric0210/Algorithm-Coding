...
{
    // 왼쪽 벽으로 쿠션 치면 벽에 맞기 전에 공이 맞는 상황
    if (startY == endY && startX > endX) return MAX_VAL;
...
{
    // 오른쪽 벽으로 쿠션 치면 벽에 맞기 전에 공이 맞는 상황
    if (startY == endY && startX < endX) return MAX_VAL;
...
{
    // 윗쪽 벽으로 쿠션 치면 벽에 맞기 전에 공이 맞는 상황
    if (startX == endX && startY > endY) return MAX_VAL;
...
{
    // 아랫쪽 벽으로 쿠션 치면 벽에 맞기 전에 공이 맞는 상황
    if (startX == endX && startY < endY) return MAX_VAL;
